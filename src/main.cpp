...
// Copyright (c) 2016 The BlackPearl developers
...
unsigned int nTargetSpacing = 60; // 1 minute
unsigned int nRetarget = 10;
unsigned int nStakeMinAge = 1 * 24 * 60 * 60; // 1 days
unsigned int nStakeMaxAge = 120 * 24 * 60 * 60; //120 days
...
const string strMessageMagic = "BlackPearl Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // BlackPearl: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // BlackPearl: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// BlackPearl: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes BlackPearl fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // BlackPearl: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BlackPearl: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // BlackPearl: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // BlackPearl: track money supply and mint amount info
...
// BlackPearl: total coin age spent in transaction, in the unit of coin-days.
...
// BlackPearl: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // BlackPearl: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // BlackPearl: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // BlackPearl: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // BlackPearl: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // BlackPearl: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // BlackPearl: check proof-of-stake
    ...
    // BlackPearl: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // BlackPearl: getblocks may not obtain the ancestor block rejected
    ...
    // BlackPearl: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BlackPearl", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BlackPearl CryptoCurrency May 2017";
        CTransaction txNew;
        txNew.nTime = 1493788488;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1493788488;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 5565021 : 16355;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("71bf00ea5fe87423d31550f1aaa2b7c9234031c952f9ed53ad177d519f596c9a"));
        block.print();
        ...
        // BlackPearl: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // BlackPearl: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // BlackPearl: tell downloading node about the latest block if it's
